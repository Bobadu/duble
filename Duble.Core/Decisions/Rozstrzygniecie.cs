...
public sealed class Rozstrzygniecie
{
    ...
    /// <summary>true = wynik taki sam jak z porownania: uzytkownik nic nie zmienial ALBO cofnal swoje zmiany
    /// (np. „to nie duplikat" i z powrotem „jednak duplikat"). Notatka albo Ignoruj = juz nie domyslna.</summary>
    ...
    public static Rozstrzygniecie Policz(Grupa g, Decyzja d)
    {
        ...
        var domZwyciezca = g.Zwyciezca ?? czlonkowie.FirstOrDefault();
        var domOdrzucone = g.Werdykt == Porownanie.Duplikat || g.Werdykt == Porownanie.Nadzbior
            ? czlonkowie.Where(x => x != domZwyciezca).ToList()
            : new List<string>();
        if (d == null) return new Rozstrzygniecie { Zwyciezca = domZwyciezca, Odrzucone = domOdrzucone, Domyslna = true };
        var r = new Rozstrzygniecie { Zwyciezca = domZwyciezca, Notatka = d.Notatka, Ignoruj = d.Ignoruj };
        ...
        r.Domyslna = !r.Ignoruj && string.IsNullOrEmpty(r.Notatka) && r.Zwyciezca == domZwyciezca
                     && r.Odrzucone.Count == domOdrzucone.Count && !r.Odrzucone.Except(domOdrzucone).Any();
        return r;
    }
