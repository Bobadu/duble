// Png.cs — minimalny koder PNG (RGB/RGBA, bez filtrow, bez przeplotu).
//
...
public static class Png
{
    ...
    public static byte[] Rgb(byte[] rgb, int w, int h)
    {
        // scanline = bajt filtra (0 = None) + w*3 bajtow
        var surowe = new byte[h * (1 + w * 3)];
        for (int y = 0; y < h; y++)
        {
            int zrodlo = y * w * 3;
            int cel = y * (1 + w * 3);
            surowe[cel] = 0;
            Buffer.BlockCopy(rgb, zrodlo, surowe, cel + 1, w * 3);
        }
