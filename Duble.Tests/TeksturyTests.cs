...
public class TeksturyTests
{
    readonly ITestOutputHelper wyj;
    public TeksturyTests(ITestOutputHelper wyj) { this.wyj = wyj; }
    ...
    public void Bc7_dekoduje_sie_do_pikseli()
    {
        ...
        foreach (var f in Directory.EnumerateFiles(folder, "*.ytd", SearchOption.AllDirectories))
        {
            ...
            if (t != null && Odciski.NazwaFormatu(t) == "BC7") { bc7 = t; plik = f; break; }
        }
        if (bc7 == null) { wyj.WriteLine("POMINIETY: nie znalazlam BC7 w civil01"); return; }
        wyj.WriteLine("BC7: " + plik);
        var px = Tekstury.Piksele(bc7, 0, out int w, out int h);
        Assert.NotNull(px);
        ...
        var px2 = Tekstury.Piksele(bc7, 2, out int w2, out int h2);
