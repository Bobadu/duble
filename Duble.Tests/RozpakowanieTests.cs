...
/// <summary>Rozpakowanie archiwum do folderu: pliki RSC7 z naglowkiem, zagniezdzone .rpf jako podfoldery, indeks kopii = indeks archiwum (pomijane bez gry).</summary>
public class RozpakowanieTests
{
    readonly ITestOutputHelper wyj;
    public RozpakowanieTests(ITestOutputHelper wyj) { this.wyj = wyj; }
    ...
    public void Archiwum_dlc_rozklada_sie_na_foldery_z_plikami_rsc7()
    {
        ...
        try
        {
            ...
            var w = Rozpakowanie.Archiwum(Sciezki.Dlc("studio_body"), Path.Combine(tmp, "studio_body"), postepy.Add);
            ...
            Assert.Contains(postepy, p => p.Etap == "rozpakuj" && p.Wszystkie > 0);
            ...
            Assert.True(Rsc7.JestRsc7(bajty)); Assert.Equal(159, Rsc7.Wersja(bajty));
            ...
            var w2 = Rozpakowanie.Zrodlo(Path.Combine(tmp, "src"), Path.Combine(tmp, "kopia"));
    ...
    public void PlikRsc7_binarny_bez_zmian_zasob_z_naglowkiem()
    {
        ...
        Assert.Same(dane, Rozpakowanie.PlikRsc7(bin, dane));
        var res = new RpfResourceFileEntry { Name = "a.ydd", SystemFlags = 0x90000000u, GraphicsFlags = 0xF0000000u };
        var wy = Rozpakowanie.PlikRsc7(res, dane);
        Assert.True(Rsc7.JestRsc7(wy));
        Assert.Equal(res.Version, Rsc7.Wersja(wy));
        Assert.Equal(dane, ResourceBuilder.Decompress(wy.Skip(16).ToArray()));
        Assert.Null(Rozpakowanie.PlikRsc7(res, null));
    }
}
