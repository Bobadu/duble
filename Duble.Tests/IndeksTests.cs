...
public class IndeksTests
{
    ...
    public void Archiwum_rpf_jako_zrodlo_daje_geometrie_i_tekstury()
    {
        ...
        var b = Zrodla.Bajty(uppr.ModelPath);
    ...
    public void Drugie_indeksowanie_bierze_odciski_z_poprzedniego_katalogu_i_pisze_miniatury()
    {
        ...
        try
        {
            ...
            Assert.All(poz1, p => Assert.False(string.IsNullOrEmpty(p.ChangeStamp)));
            Assert.All(poz1.SelectMany(p => p.Textures), t => Assert.False(string.IsNullOrEmpty(t.ChangeStamp)));
            // miniatura jest per SHA (identyczne pliki dziela jedna), wiec porownujemy z liczba ROZNYCH sha
            int roznychSha = poz1.SelectMany(p => p.Textures).Where(t => t.IsDecoded).Select(t => t.Sha256).Distinct().Count();
            Assert.InRange(Directory.GetFiles(miniatury, "*.png").Length, roznychSha * 9 / 10, roznychSha);

            var kat = new Catalog(); kat.Upsert(poz1);
            ...
            var a = poz1.OrderBy(p => p.Id).Select(p => p.Geometry.PositionHash + string.Join(",", p.Textures.Select(t => t.Sha256))).ToList();
            var b = poz2.OrderBy(p => p.Id).Select(p => p.Geometry.PositionHash + string.Join(",", p.Textures.Select(t => t.Sha256))).ToList();
            Assert.Equal(a, b);
        }
