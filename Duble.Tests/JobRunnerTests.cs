...
public class JobRunnerTests
{
    ...
    public async Task Jedno_zadanie_naraz_postep_i_koniec()
    {
        ...
        var t1 = jr.Uruchom("indeks", "A", async (ct, postep) => { start.SetResult(); await pusc.Task; postep(new Duble.Core.Postep("modele", 5, 10, "A")); });
        await start.Task;
    ...
    public async Task Postep_jest_dlawiony_do_ok_10_na_sekunde_ale_koniec_etapu_i_nowy_etap_zawsze_ida()
    {
        ...
        {
            for (int i = 0; i < 500; i++) postep(new Duble.Core.Postep("zastosuj", i, 500, "x"));   // 500 zgloszen w ulamku sekundy
            postep(new Duble.Core.Postep("zastosuj", 500, 500, null));                              // koniec etapu
            postep(new Duble.Core.Postep("porownaj", 0, 0, null));                                  // nowy etap
