...
public class ResolutionTests
{
    ...
    {
        var r = Rules.Resolve(G(Porownanie.Duplikat, "a", "a", "b", "c"), null);
        Assert.True(r.IsDefault); Assert.Equal("a", r.Winner); Assert.Equal(new[] { "b", "c" }, r.Rejected); Assert.False(r.Ignored);
        r = Rules.Resolve(G(Porownanie.Nadzbior, "b", "a", "b"), null);
        Assert.Equal(new[] { "a" }, r.Rejected);
        r = Rules.Resolve(G(Porownanie.Przemalowanie, "a", "a", "b"), null);
        Assert.Empty(r.Rejected);
        r = Rules.Resolve(G(Porownanie.DoWgladu, "a", "a", "b"), null);
        Assert.Empty(r.Rejected);
    }
