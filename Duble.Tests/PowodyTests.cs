...
public class PowodyTests
{
    ...
    public void Formatter_podstawia_parametry_i_tlumaczy_wartosci_z_malpa()
    {
        ...
        Assert.Equal("geometria identyczna, ale brak tekstur do porownania", Teksty.Powod(p, "pl"));
        Assert.Equal("identical geometry, but no textures to compare", Teksty.Powod(p, "en"));
    }

    [Fact]
    public void Formatter_odtwarza_dzisiejsze_napisy_pl()
    {
        Assert.Equal("ten sam model, te same tekstury (10/10 i 10/10 tekstur wspolnych)",
            Teksty.Powod(new Powod("SAME_MODEL_SAME_TEX", ("a", 10), ("na", 10), ("b", 10), ("nb", 10)), "pl"));
        Assert.Equal("model tylko PODOBNY (odleglosc 0.034), ale tekstury te same (3/3 i 3/3 tekstur wspolnych)",
            Teksty.Powod(new Powod("SIMILAR_MODEL_SAME_TEX", ("dist", "0.034"), ("a", 3), ("na", 3), ("b", 3), ("nb", 3)), "pl"));
        Assert.Equal("zwyciezca 93 pkt, przegrani 71, 40 pkt",
    ...
    [Fact]
    public void Punktacja_tekst_pl_jak_dawniej()
    {
        ...
        Assert.Equal("rozdzielczosc 1024px:40 | mipy 100 %:20 | wariantow 10:10 | format:10 | LOD 3:10", p.Tekst("pl"));
        p.ZlyFormat = 2; p.Format = 8;
        Assert.Equal("rozdzielczosc 1024px:40 | mipy 100 %:20 | wariantow 10:10 | format:8 (2 BC1 z alfa) | LOD 3:10", p.Tekst("pl"));
